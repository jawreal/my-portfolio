...
const PreviewProject = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      const filteredStacks = combinedStacks.filter((stack) =>
        filteredProject?.stacks?.includes(stack.id),
      );
  ...
  return (
    <div className="w-full flex justify-center border-t dark:border-slate-900">
      <div
        id="preview"
        className="w-full flex flex-col py-10 px-6 md:px-20 gap-y-6 relative max-w-7xl"
      >
        {/* Project ID number */}
        {project?.id && (
          <h1 className="absolute top-12 md:top-14 right-2 md:right-20 text-9xl font-ultra text-slate-200/80 dark:text-slate-900/75 z-1">
            {"0" + project.id}
          </h1>
        )}
        ...
        >
          <h1 className="text-3xl md:text-5xl font-ultra dark:text-slate-200 leading-none">
            {(project?.title ?? "No title found")?.toUpperCase()}
          </h1>
          <p className="text-slate-500 dark:text-slate-400 max-w-5xl">
        ...
        >
          <span className="font-extrabold dark:text-slate-200 text-sm">
            PROJECT TYPE
          </span>
          <p className="text-slate-500 dark:text-slate-400">
            {project?.type ?? "No project type found"}
          </p>
        </motion.div>
        ...
        >
          ...
          <p className="text-slate-500 dark:text-slate-400">
            {project?.date ?? "No project date found"}
          </p>
        </motion.div>

        {/* Features */}
        <div className="w-full flex flex-col gap-y-3">
          <motion.div {...animationProps(0.4, false, true)}>
            <span className="font-extrabold dark:text-slate-200 text-sm">
              FEATURES
            </span>
          </motion.div>
        ...
        <div className="w-full flex flex-col gap-y-3">
          ...
          <div
            ref={previewRef}
            className="flex flex-wrap flex-shrink-0 w-full gap-3"
          >
            {projectStacks?.map((stack: IStacks, idx: number) => {
              ...
              return (
                <motion.div
                  ...
                  key={idx}
                >
                  <Badge className="flex items-center gap-2 py-2 px-3 rounded-none bg-transparent border border-slate-300 dark:border-slate-800 shadow-none font-medium dark:text-slate-200 text-slate-950">
        ...
        {project?.credentials && (
          <div className="w-full flex flex-col gap-y-3">
            ...
            <div className="flex flex-col gap-y-1">
              <p className="text-slate-500 dark:text-slate-400">
                <span className="font-medium dark:text-slate-200 text-slate-950">
                  User/ID:{" "}
                </span>
